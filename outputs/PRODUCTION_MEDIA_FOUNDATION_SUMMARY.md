...
- `media:verify-repository`: proves intake/output roots are ignored, recognizes a broad image/RAW/video/audio extension set, rejects tracked production media, and permits only binary paths already present in the pinned approved `a5c83900` mock/reference/evidence baseline.
...
Catalog source paths must be repository-relative and inside explicit Git-ignored intake/work roots for real preparation. Absolute paths, parent traversal, non-ignored production inputs, and camera filenames such as `IMG_1234`/`DSC_0001` are rejected. Layout coordinates are not part of the contract. The authored `generatedAt`, catalog, and source hashes produce a stable source fingerprint; content hashes produce immutable semantic object keys under `v1/<scope>/<project>/<role>/`.
...
The existing mock social image was used for safe verification. Dry-run inspected 1200 × 630 and planned three 640 px formats without writing. A separate copy under ignored `media-source/masters/` exercised the real preparation path and created all three variants; inspection confirmed 640 × 336, sRGB, and no EXIF for AVIF/WebP/JPEG. The candidate contained 25 assets and validated; the derived upload plan passed a credential-free dry-run. All ignored test inputs/generated output were removed from the checkout afterward.

Candidate browser staging was also verified with `--output public/mock-media`: immutable `v1/` files landed under ignored `public/mock-media/v1/`, and both an existing mock URL and a staged candidate URL returned HTTP 200 from `npm run dev`. This local path needs a working-tree candidate manifest/content reference to display in a page; it does not change the current committed manifest or routes.
...
Explicit `--apply` is a separately authorized operation. It requires `R2_ACCOUNT_ID`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, and `R2_SECRET_ACCESS_KEY` only at runtime. For each object it performs `HEAD`; matching objects are verified/skipped, and a missing object is created atomically with `If-None-Match: *`, correct `Content-Type`, `Cache-Control: public, max-age=31536000, immutable`, and SHA-256 metadata, then `HEAD`-verified. A concurrent 412 is accepted only if the now-existing object verifies exactly. Any mismatch fails; no unconditional PUT can overwrite a hashed object.
...
- `MEDIA_URL` and `NEXT_PUBLIC_MEDIA_BASE_URL` must be both absent (default development mode) or both present with the same value; either one-sided configuration and any mismatch fails validation.
...
All positive-path commands listed below exited 0 after the final implementation. Separate one-sided environment probes intentionally exited nonzero with the required pair-consistency error.
...
- `npm run media:prepare` with an ignored-source verification catalog — three ignored derivatives, deterministic candidate, and upload plan created for pipeline verification.
- `npm run media:validate` against that candidate/catalog — 25 assets validated, including the generated strict `v1/` entry.
...
- `npm run dev` plus HTTP probes — root, existing mock asset, and ignored staged `v1/` candidate asset returned 200; the temporary dev process was stopped.
...
- Git tracking scan: production roots confirmed ignored/untracked; every tracked image/RAW/video/audio binary matches an exact path in the approved `a5c83900` mock/reference/evidence baseline.
